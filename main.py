...
class Puppy:
    def __init__(self, puppetdb_url: str, puppetdb_resource: str):
    ...
    def get_all(self):
        url = "{}/pdb/query/v4/resources/{}".format(
            self.puppetdb_url, self.puppetdb_resource
        )
        self.logger.debug("Fetching data from %s", url)
        response = requests.get(
            url,
            headers={
                "User-Agent": "puppy/{} (https://taavi.wtf/projects/puppy)".format(
                    __version__
                )
            },
        )
        ...
        for title, hosts in groupby(response.json(), lambda res: (res["title"])):
            results.append(
                {
                    "labels": {"puppet_name": title},
                    "targets": [
                        "{}:{}".format(
                            host["parameters"]["host"], host["parameters"]["port"]
                        )
                        for host in hosts
                    ],
                }
...
if __name__ == "__main__":
    puppy = Puppy(getenv("PUPPY_PUPPETDB_HOST"), getenv("PUPPY_PUPPETDB_RESOURCE"))
